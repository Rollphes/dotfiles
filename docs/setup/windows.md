...
AppData folder. The font installer scopes the registered host `USERPROFILE`,
`APPDATA` and `LOCALAPPDATA` to its PowerShell/GDI children so Windows profile
initialization cannot create `AppData` below the development home. `TEMP` and
`TMP` remain the canonical MSYS2 temp during that host operation. No runtime
data is migrated or deleted by this change.
